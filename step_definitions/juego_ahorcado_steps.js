...
// Simulamos el DOM y las funciones del juego para testing
const gameState = {
  palabraSecreta: "",
  letrasAdivinadas: [],
  intentosRestantes: 6,
...
// Funciones helper para simular el comportamiento del juego
...
function simularIngresoLetra(letra) {
  ...
  const palabraContieneLeta = gameState.palabraSecreta.toLowerCase().includes(letra.toLowerCase())
...
function obtenerProgresoPalabra() {
  return gameState.palabraSecreta
    .split("")
    .map((letra) => {
      return gameState.letrasAdivinadas.includes(letra.toLowerCase()) ? letra : "_"
    })
    .join(" ")
}
...
Given("que el juego del ahorcado está iniciado", () => {
  // El juego se considera iniciado
...
// Escenario 1: Adivinar una letra correcta
Given("que la palabra secreta contiene la letra {string}", (letra) => {
  const contieneLeta = gameState.palabraSecreta.toLowerCase().includes(letra.toLowerCase())
  expect(contieneLeta).to.be.true
})
...
// Escenario 2: Adivinar una letra incorrecta
Given("que la palabra secreta no contiene la letra {string}", (letra) => {
  const contieneLeta = gameState.palabraSecreta.toLowerCase().includes(letra.toLowerCase())
  expect(contieneLeta).to.be.false
})

Then("el progreso de la palabra no cambia", () => {
  const progreso = obtenerProgresoPalabra()
  const progresoEsperado = gameState.palabraSecreta
    .split("")
    .map(() => "_")
    .join(" ")
  expect(progreso).to.equal(progresoEsperado)
})
...
When("el progreso muestra la palabra completa", () => {
  const progreso = obtenerProgresoPalabra()
  const palabraSinEspacios = progreso.replace(/ /g, "")
  expect(palabraSinEspacios).to.equal(gameState.palabraSecreta)
})
...
Then("la palabra se muestra completamente revelada", () => {
  const progreso = obtenerProgresoPalabra()
  expect(progreso).to.not.include("_")
})

// Escenario 4: Perder el juego
Given("que he hecho {int} intentos incorrectos", (intentosIncorrectos) => {
  // Simulamos intentos incorrectos
  const letrasIncorrectas = ["z", "x", "q", "w", "k"]
  for (let i = 0; i < intentosIncorrectos; i++) {
    if (i < letrasIncorrectas.length) {
      simularIngresoLetra(letrasIncorrectas[i])
    }
  }
})

Given("me queda {int} intento restante", (intentos) => {
  expect(gameState.intentosRestantes).to.equal(intentos)
})
...
Then("el dibujo del ahorcado está completo", () => {
  // Cuando no quedan intentos, el dibujo está completo
